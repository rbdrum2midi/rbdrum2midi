...
/*static void cb_irq(struct libusb_transfer *transfer)
{
	...
	// 60 = cowbell = blue
	// 57 = cymbal = green
	// 69 = better hi hat = yellow
	// 48 = snare = red
	// 35 = bass = bass
	// 64 = tom = bass
	// 59 = hi hat = yellow
	// correct encoding for hydrogen yamaha dkit
	// 36 = kick = orange
	// 42 = open hat = black
	// 37= snare = red
	// 41= closed hat =yellow
	// 38= floortom = blue
	// 39= midtom= green
	//int drum_conn[] = {38, 39, 37, 41, 36};
	// my assignments
	//int drum_conn[] = {38, 39, 37, 41, 36};
	...
	/*
	OLD coding only for br drums, needs splitting for gh
	unsigned char *buf = irqbuf;
	unsigned char hihat   = buf[11];
	unsigned char snare   = buf[12];
	unsigned char cymbal  = buf[13];
	unsigned char cowbell = buf[14];
	unsigned char bass    = buf[0] & 0x10;
	
	unsigned char bass_down = 0;
	//added signals
	unsigned char bass2   = buf[0] & 0x20;
	unsigned char bassgh  = buf[15];	
	...
	if(kit==1){
		...
		unsigned char hihat   = buf[11]; //Yellow
		unsigned char snare   = buf[12]; //Red
		unsigned char cymbal  = buf[13]; //Green
		unsigned char cowbell = buf[14]; //Blue
		unsigned char bass    = buf[0] & 0x10; //Orange Pedal
		unsigned char bass2   = buf[0] & 0x20; //Black Pedal
		unsigned char bass_down = 0;
		int velocity;
		if (snare || drum_state[0]) {
			// Events:
			// Down
			if (snare && !drum_state[0]) {
				velocity = snare * 2;
				velocity = min(max(velocity, 0), 127);
				noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[2], 0);
				notedown(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[2], velocity);
				}
		
			drum_state[0] = snare;
			}
		//yellow pad and cymbal	
		if (hihat || drum_state[1]) {
			// Events:
			// Down
			if (hihat && !drum_state[1]) {
				velocity = hihat * 2;
				velocity = min(max(velocity, 0), 127);
                if(buf[1] & 0x08){
					noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[4], 0);
					notedown(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[4], velocity);
					}
				else{
					noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[3], 0);
					notedown(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[3], velocity);
					}
				}
		...
		if (cowbell || drum_state[2]) {
			...
			if (cowbell && !drum_state[2]) {
				...
				velocity = min(max(velocity, 0), 127);
				if(buf[1] & 0x08){
					noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[6], 0);
					notedown(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[6], velocity);
					}
				else{
					noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[5], 0);
					notedown(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[5], velocity);	
					}
				}
		
			drum_state[2] = cowbell;
			}
		//Green pad and Cymbal
		if (cymbal || drum_state[3]) {
			// Events:
			// Down
			if (cymbal && !drum_state[3]) {
				velocity = cymbal * 2;
				velocity = min(max(velocity, 0), 127);
				if(buf[1] & 0x08){
					noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[8], 0);
					notedown(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[8], velocity);
					}
				else{	
					noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[7], 0);
					notedown(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[7], velocity);
					}	
				}
		...
		if (bass || drum_state[4]) {
			...
			if (bass && !drum_state[4]) {
				...
				velocity = 127;
				velocity = min(max(velocity, 0), 127);
				notedown(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[0], velocity);
				}
			// Up
			if (!bass && drum_state[4]) {
				noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[0], 0);
				}
			drum_state[4] = bass;
			}
		// Pedal 2 (black)	
		if (bass2 || drum_state[5]) {
			...
			if (bass2 && !drum_state[5]) {
				...
				velocity = 127;
				velocity = min(max(velocity, 0), 127);
				notedown(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[1], velocity);
				}
			// Up
			if (!bass2 && drum_state[5]) {
				noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[1], 0);
				}
			drum_state[5] = bass2;
			}		
			if (verbose && (snare || hihat || cowbell || cymbal || bass_down)) {
			printf("%s %s %s %s %s %s \n", snare>0?"VV":"  ", hihat>0?"VV":"  ", cowbell>0?"VV":"  ", cymbal>0?"VV":"  ", bass>0?"VV":"  ", bass2>0?"VV":"  ");
			printf("%02i %02i %02i %02i %02i %02i\n", snare, hihat, cowbell, cymbal, bass, bass2);
	...
	if(kit==2){	
		...
		//Pedal
		if ((bassgh || drum_state[6]) & kit==2) {
			// Events:
			// Down
			if (bassgh && !drum_state[6]) {
				bass_down = 1;
				velocity = bassgh * 2;
				velocity = min(max(velocity, 0), 127);
				noteup(g_seq, g_port, DEFAULT_CHANNEL, drum_conn[0], 0);
			...
			if (verbose && (snare || hihat || cowbell || cymbal || bass_down || ride)) {
                printf("%s %s %s %s %s %s\n", snare>0?"VV":"  ", hihat>0?"VV":"  ", cowbell>0?"VV":"  ", cymbal>0?"VV":"  ", bassgh>0?"VV":"  ", ride>0?"VV":"  ");
			printf("%02i %02i %02i %02i %02i %02i\n", snare, hihat, cowbell, cymbal, bassgh, ride);
			printf("%02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x\n", 
			buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7], buf[8], buf[9], 
			buf[10], buf[11], buf[12], buf[13], buf[14], buf[15], buf[16], buf[17], buf[18], buf[19], 
			buf[20], buf[21], buf[22], buf[23], buf[24], buf[25], buf[26]);
